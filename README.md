...
PRIDE stores $K+1$ plaintexts (one real, $K$ decoys) inside a single ciphertext $D$ — a list of field elements. Encrypting a new plaintext $m$ picks $n-2$ existing shares from $D$, solves for the unique degree-$(n-2)$ polynomial passing through those shares with $p(0) = m$, then appends one new evaluation point. Each secret key holds the resulting $n-1$ reconstruction points. Decryption with any of the $K+1$ secret keys recovers the corresponding plaintext via Lagrange interpolation at zero over those $n-1$ points. The scheme requires $n \geq 3$; the corner case $n = 2$ collapses to a degree-$0$ polynomial (a constant) and provides no secrecy.
...
| Secret sharing | $(n-1, n)$ Shamir with share reuse (polynomial degree $n-2$, threshold $n-1$, requires $n \geq 3$) |
| Storage overhead | $K+1$ shares per real record |
| Encryption complexity | $O(n^3)$ per record (Gaussian elimination on an $(n-2) \times (n-2)$ Vandermonde-like system) |
| Retrieval complexity | $O(n^2)$ per key (Lagrange interpolation over $n-1$ points) |
...
|---|---|---|
| `n` | `int` | Scheme parameter from the $(n{-}1, n)$ label. Reconstruction threshold is $n-1$; the sharing polynomial has degree $n-2$. Requires $n \geq 3$. |
| `K` | `int` | Number of decoys per real record |
| `p` | `int` | Field prime (default: $2^{61}-1$) |
| `init_shares` | `int` | Random shares pre-loaded into $D$ on setup. Must satisfy `init_shares >= n-2`; for $n \geq 8$ pass `init_shares=n` (or higher) explicitly, since the default of $5$ is too small. |
...
Sweeps $n \in \{3,5,7,10\}$ and $K \in \{1,2,5,10,20\}$ at $N=10{,}000$ synthetic Gaussian records. Reports encryption time per record, retrieval time, and shares per record. ($n=2$ is excluded because the degree-$0$ sharing polynomial is degenerate; the notebook sets `init_shares=max(5, n)` so larger $n$ values work without a manual override.)
