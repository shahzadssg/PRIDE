...
pride_core.py
=============
Shared PRIDE implementation used by all four R2 experiment notebooks.

Implements:
  - Modular arithmetic in F_p with p = 2^61 - 1 (Mersenne prime)
  - (n-1, n) Shamir secret sharing with share-reuse:
      threshold = n - 1, polynomial degree = n - 2,
      reconstruction needs n - 1 points
  - PRIDE.Setup / Enc / Dec / Retrieve from the paper (R2 revision)

The public API exposes one class, PRIDE, plus a couple of helpers.

Conventions match the R2 manuscript exactly:
  - n  = Shamir threshold (paper's n); polynomial degree is n - 2
  - K  = number of decoys per real record
  - D  = ciphertext (list of field elements, the alpha_i)
  - sk = (I_c, I_r) where I_c = list of n-1 x-coordinates,
         I_r = list of n-1 key-base indices (kept here for API parity
         even though we don't use the key base for the experiments)

Implementation note:
  The R2 algorithm picks (n-2) shares from D plus the
  hidden plaintext m at x=0 to determine a polynomial of degree (n-2), then
  evaluates at one new x to produce the new share. sk holds n-1 points.
  n >= 3 is required: n=2 gives a degree-0 polynomial (a constant) and is
  trivially insecure.

Author: Shahzad Ahmad / Stefan Rass (R2 revision artifacts)
"""
...
def solve_linear_for_poly(xs: List[int], ys: List[int], m: int,
    ...
    """
    Given (n-2) data points (xs[j], ys[j]) and the constant term m = p(0),
    solve for the (n-1) coefficients (a_0=m, a_1, ..., a_{n-2}) of the
    degree-(n-2) polynomial that interpolates all (n-1) constraints
    (the n-2 supplied points plus p(0) = m).

    Returns the coefficient list [a_0, a_1, ..., a_{n-2}] mod p.

    The free linear system is (n-2) x (n-2) on the unknowns a_1..a_{n-2},
    Vandermonde-like in the supplied xs.
    """
    n_minus_2 = len(xs)
    # We want p(x) = m + a_1 x + a_2 x^2 + ... + a_{n-2} x^{n-2}
    # and p(xs[j]) = ys[j] for j = 0..n-3
    # i.e. for each j: sum_{k=1}^{n-2} a_k * xs[j]^k = ys[j] - m
    # That's a (n-2) x (n-2) Vandermonde-like system.
    # We solve it via Gaussian elimination in F_p.
    A = [[f_mul(1, pow(xs[j], k, p), p) for k in range(1, n_minus_2 + 1)]
         for j in range(n_minus_2)]
    b = [(ys[j] - m) % p for j in range(n_minus_2)]
    # Forward elimination
    for col in range(n_minus_2):
        # Find pivot
        pivot_row = None
        for r in range(col, n_minus_2):
            if A[r][col] != 0:
                pivot_row = r
                break
...
@dataclass
class PRIDEParams:
    n: int           # Shamir threshold; polynomial degree is n - 2; reconstruction needs n - 1 points
    K: int           # decoys per real record
    p: int = P_FIELD
    init_shares: int = 5   # number of random initial shares in D; must be >= n-2
...
class PRIDE:
    ...
    def __init__(self, params: PRIDEParams, seed: int | None = 40):
        if params.n < 3:
            raise ValueError(
                f"PRIDE requires n >= 3 (got n={params.n}); n=2 yields a "
                "degree-0 polynomial = constant, which provides no secrecy."
            )
        self.params = params
        self.rng = np.random.default_rng(seed)
        self.cs_rng = secrets.SystemRandom()  # for indices (uniform)
        # Key base r: kept for API parity; not used in 0-DP analysis.
        # Size 2n is arbitrary; matches the PRIDE.Setup k ~ U{n,...,2n} range.
    ...
    def encrypt_single(
        ...
        Steps (mirror Algorithm 2 of the R2 paper):
          1. Pick (n-2) distinct existing indices i_1..i_{n-2} from D.
          2. Sample a_1..a_{n-2} as the unique solution to the (n-2) linear
             constraints p(i_j) = D[i_j] together with p(0) = m.
             The resulting polynomial p has degree n-2.
          3. New share alpha_{ell+1} = p(ell+1). Append to D.
          4. sk = ({i_1, ..., i_{n-2}, ell+1}, dummy I_r); |I_c| = n-1.
        """
        n = self.params.n
        p = self.params.p
        ell = ct.length
        # Need at least n-2 existing shares; PRIDE.Setup populates init_shares
        # >= n random elements so this holds at the start.
        if ell < n - 2:
            raise ValueError(f"Need at least n-2={n-2} shares in D; have {ell}.")
        # Pick (n-2) distinct existing indices uniformly without replacement.
        ...
        # NOTE: the case n=3 means we pick exactly 1 existing share + 1 new.
        I_c_existing = sorted(
            self.rng.choice(ell, size=n - 2, replace=False).tolist()
        )
        ...
        I_c = xs + [new_x]                # the (n-1) x-coordinates
        # Dummy I_r of size n-1 — not used in 0-DP analysis. Use numpy for speed.
        I_r = self.rng.choice(len(self.r), size=n - 1, replace=False).tolist()
        return (I_c, I_r)
    ...
    def dec(self, ct: PRIDECiphertext, sk: SecretKey) -> int:
        """PRIDE.Dec: Lagrange-interpolate at z=0 over the n-1 shares of sk."""
        I_c, _ = sk
        xs = list(I_c)
